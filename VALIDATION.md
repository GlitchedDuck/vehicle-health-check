...
- Actual GLB container and scene parsing: passed. The new model contains Body and four custom marker anchors; its geometry buffer is embedded.
- All four module constructors: passed with real Three.js geometry.
- Repeated brake selection while the vehicle is faded: exercised using the procedural wheel fallback.
...
Car_v2 conversion: 97,536 triangles retained; indexed GLB approximately 2.4 MB. Actual GLB parser, all four marker-to-surface distance checks and procedural wheel fallback passed. Geometry was inspected with an offline render; browser visual testing was not performed.
